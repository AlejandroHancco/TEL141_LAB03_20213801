#!/usr/bin/env bash
# ==========================================================
# Script: vm_orchestrator_fase2.sh
# Propósito: Configura HeadNode, Namespaces y OFS
# ==========================================================

set -euo pipefail

USER="ubuntu"
PASS="ubuntu123"
REMOTE_DIR="/home/ubuntu/TEL141_LAB03_20213801"

HEADNODE_HOST="10.0.10.4"
OFS_HOST="10.0.10.5"
SSH_OPTS=(-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null)

execute_remote() {
    local host="$1"
    local cmd="$2"
    sshpass -p "$PASS" ssh "${SSH_OPTS[@]}" "$USER@$host" "cd $REMOTE_DIR && echo $PASS | sudo -S bash -c '$cmd'"
}

echo "==> Inicializando HeadNode..."
execute_remote "$HEADNODE_HOST" "./init_headnode.sh br-int ens4"

echo "==> Creando namespaces VLAN..."
execute_remote "$HEADNODE_HOST" "
./ns_create.sh vlan100 br-int 100 '192.168.100.10-192.168.100.50' 192.168.100.1
./ns_create.sh vlan200 br-int 200 '192.168.200.10-192.168.200.50' 192.168.200.1
./ns_create.sh vlan300 br-int 300 '192.168.230.10-192.168.230.50' 192.168.230.1
"

echo "==> Inicializando OFS..."
execute_remote "$OFS_HOST" "./init_ofs.sh br-ofs ens5 ens6 ens7 ens8"

echo "==> Estado final:"
execute_remote "$HEADNODE_HOST" "ovs-vsctl show"
execute_remote "$OFS_HOST" "ovs-vsctl show"

echo "✅ Fase 2 completada correctamente"
